use crate::error::SavingsVaultError;
...
pub fn assert_keys_equal(key1: Pubkey, key2: Pubkey) -> Result<()> {
    if sol_memcmp(key1.as_ref(), key2.as_ref(), PUBKEY_BYTES) != 0 {
        err!(SavingsVaultError::PublicKeyMismatch)
    } else {
        Ok(())
    }
}
...
pub fn assert_initialized<T: Pack + IsInitialized>(account_info: &AccountInfo) -> Result<T> {
    ...
    if !account.is_initialized() {
        Err(SavingsVaultError::UninitializedAccount.into())
    } else {
        Ok(account)
    }
}
...
pub fn assert_owned_by(account: &AccountInfo, owner: &Pubkey) -> Result<()> {
    if account.owner != owner {
        Err(SavingsVaultError::IncorrectOwner.into())
    } else {
        Ok(())
    }
}
...
pub fn assert_derivation(program_id: &Pubkey, account: &AccountInfo, path: &[&[u8]]) -> Result<u8> {
    ...
    if key != *account.key {
        return Err(SavingsVaultError::DerivedKeyInvalid.into());
    }
    Ok(bump)
}
...
pub fn close_account<'a>(
    ...
    **receiver_account.lamports.borrow_mut() = receiver_account
        ...
        .ok_or(SavingsVaultError::NumericalOverflow)?;
